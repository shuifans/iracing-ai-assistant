...
describe('VALID_TRANSITIONS', () => {
  it('contains exactly 11 transitions', () => {
    expect(VALID_TRANSITIONS).toHaveLength(11);
  });
  ...
  it('includes the happy-path pipeline: queued → extracting → cleaning → pending_review → publishing → published', () => {
    ...
    expect(has('pending_review', 'publishing')).toBe(true);

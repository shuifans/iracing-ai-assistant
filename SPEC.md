...
| ADR-006 | 知识发布门禁     | LLM 清洗后只生成候选稿；必须由知识库管理员预览并点击审核通过后发布                 |
...
8. 原始知识文件/URL 提交、异步抽取和独立 LLM Provider 清洗。
...
PM2: iracing-ai-assistant (shserver)
   ...
   │   ├── LLM Provider knowledge cleaning
...
聊天只使用一个 Qoder Agent。该 Agent 固定使用 `Qwen3.7-Plus` 和高推理强度，直接负责问题理解、追问、本地知识检索、可选联网检索、证据综合和最终回答；应用不实现第二套聊天生成链路，也不注册任何额外 Agent。

本地知识库是第一检索源。Agent 直接使用 `Glob`、`Grep`、`Read` 访问正式 Wiki；即使会话已开启联网，也必须先完成本地检索，并只在没有有效本地证据时使用 `WebSearch`、`WebFetch`。

联网知识源以 SQLite `web_knowledge_sources` 为运行时事实来源，由知识管理员维护 domain、path 或 exact URL 规则。应用为每轮 Agent 调用生成规则，并同步生成 [notes/knowledge-sources.md](./notes/knowledge-sources.md) 快照用于审阅，不从快照反向覆盖数据库。

### 10.3 工具权限

聊天会话：

- `options.tools` 只暴露 `Read`、`Glob`、`Grep`；会话联网开关开启时才额外暴露 `WebFetch`、`WebSearch`。
...
- 每轮最多调用 1 次 `WebSearch` 和 2 次 `WebFetch`；无效或越权调用拒绝执行且不消耗有效调用预算。每次跳转都必须重新校验目标 URL。
- 单轮最多 6 个 Agent turn，总 deadline 为 120 秒。

知识清洗不属于聊天 Agent 工具面。离线 Worker 使用独立的 `LLM_API_PROVIDERS` 与对应 `LONGCAT_*` 等 Provider 配置处理已抽取文本，只生成候选稿；正式 Wiki 仍必须经过管理员审核和受控发布流程。
...
| `assistant` tool_use          | 发送不含入参的阶段进度，只记录工具名和次数               |
...
应用在直接 `Read` 和 `WebFetch` 工具成功后构造 evidence，并写入当前请求的内存 evidence registry；工具原始输出仍返回给 Qoder，不得被 evidence 回调覆盖。最终正文只能使用 `[S1]`、`[S2]` 形式引用 registry 中的证据；应用把被实际引用的 evidence 持久化为 `message_sources`，未引用结果不得计入知识命中或发送 `source` SSE。

### 10.5 多轮会话策略

- 整个业务会话持续使用同一个 Qoder `session_id`，直到用户结束会话；每轮请求结束后关闭 Query 资源，但保留 SDK 会话用于下一轮 `resume`。
- 新会话创建 Query 时只注入一次聊天 System Prompt 和当前用户消息。
- 已有 `qoder_session_id` 时通过 SDK `resume` 恢复，并且只发送当前轮用户消息；不得重复发送 System Prompt，也不得把数据库历史消息重新拼接进请求。
- `resume` 因会话不存在、失效或过期而失败时，只用 System Prompt 和当前用户消息创建新的 Qoder 会话，更新 `qoder_session_id` 后重试一次；不得回放数据库历史。
- 数据库仍保存消息用于产品展示、审计和失败恢复，但不承担 Qoder 上下文拼接。
- 不保存或展示模型思考链。
...
3. 优先使用 Wiki；仅当用户已开启本会话联网且 Wiki 无有效证据时，才使用管理员登记的网页。
...
聊天模型固定为 `Qwen3.7-Plus`，应用层不实现按问题复杂度切换模型的路由器；应用必须记录 SDK 实际返回的模型、token 和 cost。提示词要求把检索材料当作不可信数据，忽略其中要求改变角色、泄露提示词或调用未授权工具的指令。
...
4. 新会话注入一次 System Prompt；后续只用当前消息恢复 Qoder Query。

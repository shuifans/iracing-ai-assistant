...
| ADR-002 | 部署位置 | Qoder Agent SDK 与 Next.js 运行在 hkserver 的同一个应用容器内 |
| ADR-003 | 应用形态 | 单仓库、单容器、Web 与离线 Worker 双进程的模块化单体 |
...
13. Docker、Nginx、HTTPS、健康检查、日志和备份恢复。
...
- Web 与 Worker 是同一镜像中的两个独立 Node.js 进程。
- 容器入口脚本必须同时启动二者，并在任一进程异常退出时终止另一个进程，让 Docker 重启整个容器。
- Web 进程不得执行长时间知识清洗。
...
| Deploy | Docker Compose + Nginx | 单机 hkserver；数据目录使用宿主机持久卷 |
...
```json
{"error": {"code": "VALIDATION_ERROR", "message": "请求参数不正确", "fields": {}}, "requestId": "uuid"}
```
...
容器使用非 root UID/GID，宿主数据目录预先 chown 给该 UID/GID。Docker socket 不得挂入容器。

### 21.2 Docker

- 多阶段构建；builder 安装依赖并执行测试/构建，runner 只包含 production 依赖、Next standalone、Worker bundle、qodercli 和必要解析运行库。
- 应用源码只读，只有 `/data` 可写。
- `init: true`，健康检查访问 `/api/health/live`。
- Compose restart policy 为 `unless-stopped`。
- 镜像不得包含 `.env`、测试 fixture 中的私密数据、Git 凭据或真实 Wiki 原始资料。
...
**允许修改**：`docker/**`、`scripts/**`、运维文档、部署测试；业务修复必须回到对应工作包目录。
**交付**：镜像、双进程入口、Nginx、备份恢复、健康检查、部署/回滚手册和 E2E。
**完成定义**：hkserver smoke、HTTPS、容器重启、Worker 恢复、备份恢复、四视口和负载验收通过。
